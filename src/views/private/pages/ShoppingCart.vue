  ...
  <div>
    <div class="container content">
      ...
      <div class="row">
        ...
        <div class="col-6">
          <h4 class="">
            Total: <b>{{ formatCurrency(totalSum) }}</b>
          </h4>
        </div>

        <div class="col-6">
          <button
            class="btn btn-info btn-sm float-end text-white"
        ...
        <div class="col-12">
          <div v-if="payData.allItems.length > 0">
            <div class="list-all mb-2">
              <div
                v-for="item in payData.allItems"
                :key="item.id"
                class="list-item"
              >
                <div class="item-details">
                  <div class="item-info">
                    <b>{{ item.name }}</b>
                    <span class="item-price" @click="removeItem(index)"
                  ...
                  <div class="item-info">
                    <span class="item-available"
                      >Available: {{ item.number }}</span
                    >
                    <span class="item-price"
                      >Unit Price: {{ formatCurrency(item.amount_sold) }}</span
                    >
                  </div>
                  <div class="item-info">
                    <span class="">
                    ...
                    <span class="item-price">
                      Total Price:
                      {{ formatCurrency(item.amount_sold * item.qty) }}
                    </span>
                  </div>
                </div>
              </div>
      ...
      >
        <form>
          <div class="mb-2">
            <label class="form-label text-gray fw-bold" for="product-name"
              >Product Name</label
            >
            <Dropdown
              v-model="productData"
              :options="bars?.[bar_id]?.['items']"
              filter
              optionLabel="name"
          ...
          <button
            @click.prevent="addToCart(productData)"
            type="submit"
            class="btn w-100 btn-success btn-block"
          >
            Submit
          </button>
        </form>
...
onMounted(() => {
  bar_id.value = JSON.parse(localStorage.getItem("bar_id"));
