<template>
	<div class="content-wrapper">
		<loading :active="isLoading || isLoading2 || isLoading3 || isLoading4"/>
		<div class="container mt-3">
			...
			<div class="text-center justify-content-between align-items-center mt-3">
				<span class="mx-3 border p-3"><b>Total Count</b>: {{ total_count }} Sale<span v-if="total_count > 1">s</span></span> 
				<span class="mx-3 border p-3"><b>Total Amount</b>: {{ formatCurrency(total_amount) }}</span>
			</div>

			<div class="my-3" :key="reportsData">
				<table ref="myTable" class="text-start mb-3 shadow">
					<thead class="text-uppercase bg-secondary text-white">
						<tr>
							<th scope="col" class="p-3">Bar</th>
							<th scope="col" class="p-3">Payment</th>
							<th scope="col" class="p-3">Customer</th>
							<th scope="col" class="p-3">Steward</th>
					...
					<tbody class="" v-if="!isLoading">
						...
						>
							...
							<td class="p-3" style="width: 10%">
								{{ data["mode_of_payment"] }} <br>
								{{ data["channel"]["name"] }}
							</td>
							<td class="p-3" style="width: 10%">
								{{ data["member"] !== null ? data["member"]["first_name"] + " " + data["member"]["last_name"] : data["type_of_member"] }}
							</td>
		...
		const getReportData = async () => {
			...
			await axiosUrl
				...
				.then((response) => {
					...
					total_count.value = responseData.total_count;
					total_amount.value = responseData.total_amount;
