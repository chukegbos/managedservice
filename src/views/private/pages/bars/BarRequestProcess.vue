      ...
      <div class="container" v-if="!isLoading">
        ...
        <div class="" v-if="status_sent == 0">
          <div class="row mb-3">
            ...
            <div class="col-md-8" v-if="canCreate()">
              <form @submit.prevent="onSubmit()">
                <table class="table table-bordered table-responsive-md">
                  ...
                  <tbody>
                    <tr v-for="item in productItems" :key="item">
                      ...
                      <td>
                        <select
                          v-model="item.requestFrom"
                          class="form-control"
                          required
                        >
                          <option value="0">Store (Inventory)</option>
                          <option
                            v-for="option in bars"
                            :value="option.bar_code"
                            :key="option.id"
                          >
                            {{ option.name }}
                          </option>
                ...
                <div class="my-3">
                  <button
                    class="btn btn-primary account-btn w-100"
                    type="submit"
                  >
                    Submit
                  </button>
...
const onSubmit = async () => {
  ...
  productItems.value.forEach(function (product) {
    if (product.quantity == null) {
      error.push(product);
      Swal.fire("Failed!", "Fill all the quantity fields", "warning");
    }

    if (product.requestFrom == null) {
      error.push(product);
      Swal.fire("Failed!", "Fill all the source fields", "warning");
    }
  });
