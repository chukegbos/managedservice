<template>
    <div class="content-wrapper">
        ...
        <div class="container">
            ...
            <form @submit.prevent="getDebt" class="row">
                <div class="col-md-4">
                    <label for="">Start Date</label>
                    <input type="date" id="startDate" v-model="startDate" class="form-control" @change="validateDates"/>
                </div>

                <div class="col-md-4">
                    <label for="">Password</label>
                    <input type="date" id="endDate"  v-model="endDate" class="form-control" @change="validateDates"/>
                </div>
                <div class="col-md-4">
                    <button type="submit" class="btn btn-primary my-3" :disabled="!isValid">
                        Filter
                    </button>
                </div>
            </form>
                ...
                <div v-else>
                    <div class="card card-body">
                        <div class="alert alert-warning" role="alert">
                            <p class="text-center">No debt available</p>
                        </div>
                    </div>
                </div>
            </div>
...
<script setup>
    ...
    const validateDates = () => {
      ...
      isValid.value = startDate.value && endDate.value && startDate.value <= endDate.value;
    };
    ...
    const getDebt = async () => {
        if (isValid.value) {
            ...
            await axiosUrl
            .get('/report/membership/debt/?startDate='+startDate.value+'&endDate='+endDate.value)
            .then((response) => {
